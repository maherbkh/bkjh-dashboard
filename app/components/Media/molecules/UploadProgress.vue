  ...
  <div :class="['space-y-3', props.class]">
    ...
    <div class="flex items-center gap-3">
      ...
      <div class="flex-1 min-w-0">
        ...
        <p v-if="filename" class="text-xs text-muted-foreground truncate">
          {{ filename }}
        </p>
      </div>
      <div v-if="status === 'uploading'" class="text-xs text-muted-foreground">
        {{ Math.round(progress) }}%
      </div>
    </div>

    <!-- Progress bar -->
    <div v-if="status === 'uploading' || status === 'processing'" class="space-y-2">
      <Progress :value="progress" class="h-2" />
      
      <div v-if="formattedSpeed || formattedRemainingTime" class="flex justify-between text-xs text-muted-foreground">
    ...
    <!-- Complete state -->
    <div v-if="status === 'complete'" class="flex items-center gap-2 text-green-600 dark:text-green-400">
      <Icon name="solar:check-circle-outline" class="w-4 h-4" />
      <span class="text-sm font-medium">{{ t('media.upload_successful') }}</span>
    </div>

    <!-- Error state -->
    <div v-if="status === 'error'" class="flex items-center gap-2 text-red-600 dark:text-red-400">
