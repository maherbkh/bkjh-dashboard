...
<template>
  <div :class="['space-y-3', props.class]">
    <!-- Basic info -->
    <div class="space-y-2">
      <h3 class="font-semibold text-lg">{{ media.filename }}</h3>
      
      <div v-if="media.title" class="text-sm text-muted-foreground">
        <strong>{{ t('media.title') }}:</strong> {{ media.title }}
      </div>
      
      <div v-if="media.description" class="text-sm text-muted-foreground">
        <strong>{{ t('media.description') }}:</strong> {{ media.description }}
      </div>
      
      <div v-if="media.altText" class="text-sm text-muted-foreground">
    ...
    <div class="grid grid-cols-2 gap-4 text-sm">
      ...
      <div>
        ...
        <MediaSize :size="media.size" class="ml-2" :show-icon="false" />
      </div>
    ...
    <div v-if="showAccessLevel || showCollection" class="flex items-center gap-4">
      <MediaBadge
      ...
      <div v-if="showCollection && media.collectionName" class="text-sm text-muted-foreground">
    ...
    <!-- Metadata -->
    <div v-if="showMetadata && Object.keys(media.metadata).length > 0" class="space-y-2">
      <h4 class="font-medium text-sm">{{ t('media.metadata') }}</h4>
    ...
    <div class="text-xs text-muted-foreground space-y-1">
      <div>
        ...
        <MediaDate :date="media.createdAt" format="absolute" class="ml-2" :show-icon="false" />
      </div>
      
      <div v-if="media.updatedAt !== media.createdAt">
        <strong>{{ t('media.updated_at') }}:</strong>
        <MediaDate :date="media.updatedAt" format="absolute" class="ml-2" :show-icon="false" />
      </div>
    </div>
  </div>
</template>
