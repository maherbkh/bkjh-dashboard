  ...
  >
    ...
    <div class="flex-1 min-w-0">
      <div class="flex items-center gap-2 mb-1">
        <h4 class="font-medium text-sm truncate">{{ media.filename }}</h4>
        <MediaBadge :access-level="media.accessLevel" size="sm" />
      </div>
      
      <div class="flex items-center gap-3 text-xs text-muted-foreground">
        <MediaSize :size="media.size" />
        <MediaDate :date="media.createdAt" />
        <span v-if="media.collectionName" class="capitalize">
          {{ media.collectionName }}
        </span>
      </div>
      
      <div v-if="media.title || media.description" class="mt-1">
        <p v-if="media.title" class="text-xs text-muted-foreground truncate">
          {{ media.title }}
        </p>
        <p v-if="media.description" class="text-xs text-muted-foreground truncate">
          {{ media.description }}
        </p>
      </div>
    </div>
    ...
    >
      <Button
        ...
        @click="handleEdit"
      >
        <Icon name="solar:pen-outline" class="w-4 h-4" />
      </Button>
      <Button
        size="sm"
        variant="ghost"
