        ...
        >
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div class="lg:col-span-9 space-y-6">
                    ...
                    <div v-if="ticket.actionsHistory && ticket.actionsHistory.length > 0">
                        <PageTable
                            ...
                            <template #cell-note="{ row }">
                                ...
                                >
                                    <HoverCard v-if="row.note">
                                        <HoverCardTrigger class="line-clamp-2 max-w-64 cursor-pointer hover:text-primary ease-in-out duration-300">
                                            {{ row.note }}
                                        </HoverCardTrigger>
...
// Fetch ticket data
import type { Ticket, TicketAction, TicketStatus, TicketCrudForm, Attachment } from '~/types';
...
const { data: ticketData, pending: isLoading, error, refresh } = await useApiFetch<{
    status: string;
    message: string;
    data: Ticket & { attachments?: Attachment[] };
}>(`/api/ticket/${props.uuid}`);

const ticket = computed(() => ticketData.value?.data as (Ticket & { attachments?: Attachment[] }) | undefined);
...
watchEffect(() => {
    if (ticket.value && ticket.value.ticketNumber) {
        addOpenedTicket(props.uuid, ticket.value.ticketNumber);
    }
});

// Clean up when component is unmounted
onUnmounted(() => {
    removeOpenedTicket(props.uuid);
});
...
const handleEdit = () => {
    ...
    setValues({
        ...
        cell: ticket.value.cell || '',
        deviceId: ticket.value.deviceId || '',
        groupId: ticket.value.group?.id || null,
        categoryId: ticket.value.category?.id || null,
        status: ticket.value.status,
    });
    isEditDialogOpen.value = true;
};

const onEditSubmit = async (values: TicketCrudForm) => {
    if (!ticket.value) return;
...
const getLatestStatus = (statuses: TicketStatus[]) => {
    ...
    return statuses.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];
};
...
const assignSelf = async () => {
    ...
    try {
        await useApiFetch(`/api/ticket/${ticket.value.uuid}/self-assign`, {
            method: 'POST',
        });
...
const transferTicket = async (userId: number) => {
    ...
    isActionLoading.value = true;
    try {
        await useApiFetch(`/api/ticket/${ticket.value.uuid}/transfer`, {
            method: 'POST',
            body: { user_id: userId },
        });
...
const addTicketAction = async (actionType: string, note?: string) => {
    ...
    isActionLoading.value = true;
    try {
        await useApiFetch(`/api/ticket/${ticket.value.uuid}/action`, {
            method: 'POST',
            body: {
...
const actionTypes = [
    'assign', 'reassign', 'unassign', 'temporary_assign',
    'upgrade_hardware', 'downgrade_hardware', 'upgrade_software', 'downgrade_software',
    'repair_hardware', 'repair_software', 'clean', 'reconfigure',
    'change_network', 'add_peripherals', 'remove_peripherals',
    'install_software', 'uninstall_software', 'update_license',
    'decommission', 'reactivate', 'mark_off_duty', 'return_to_inventory',
    'troubleshoot', 'reset_password', 'replace', 'audit',
    'backup', 'restore', 'loan', 'retrieve', 'tag',
    'create', 'status_change', 'transfer',
];
...
const sortedActionsHistory = computed(() => {
    if (!ticket.value?.actionsHistory) return [];
    return [...ticket.value.actionsHistory].sort((a: TicketAction, b: TicketAction) =>
