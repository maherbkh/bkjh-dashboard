...
const handleExportAttendees = async () => {
  ...
  try {
    ...
    // Use direct fetch for file download (useApiFetch doesn't handle blob responses well)
    // Using the same path structure as useApiFetch (/backend prefix)
    const response = await fetch(
      `/backend/academy/events/${props.event.id}/registrations/export`,
      {
        method: "GET",
        headers: {
    ...
    let filename = `event-${props.event.id}-attendees.xlsx`;
...
<template>
  <div class="flex flex-col gap-6">
    ...
    >
      <div
        class="flex items-center justify-center size-20 rounded-full bg-muted mb-6"
      >
        <Icon
      ...
      <h2 class="text-2xl font-semibold text-foreground mb-2">
        {{
          $t("action.message.not_found_title", {
            model: $t("academy.singular"),
          })
        }}
      </h2>
      <p class="text-muted-foreground mb-6 max-w-md">
        {{
          $t("action.message.not_found_description", {
            model: $t("academy.singular"),
          })
        }}
      </p>
    ...
    <!-- Event Content -->
    <div v-else-if="props.event" class="space-y-6">
      <!-- Header -->
      <div
        class="flex lg:flex-row flex-col gap-5 lg:items-center justify-between"
      >
          ...
          <div>
            <div class="text-lg font-bold flex items-center gap-4">
              ...
              <Icon
                ...
                "
                :class="[
                  event.isActive ? 'text-success' : 'text-muted-foreground',
                  'shrink-0 size-5',
                ]"
              />
            </div>
        ...
        <div class="xl:col-span-4 space-y-6">
          <Card>
            ...
            <CardContent class="flex flex-col divide-y divide-dashed">
              <AppListItem :title="$t('academy.type')" :value="event.type" />
              <AppListItem
              ...
              <AppListItem :title="$t('academy.room')" :value="event.room" />
