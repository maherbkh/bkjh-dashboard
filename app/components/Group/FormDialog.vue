...
const dialogDescription = computed(() => {
  return props.dialogMode === "add" ? t("group.add") : t("group.edit");
});
...
<template>
  <FormDialog v-model:open="isOpen" :title="dialogTitle" :description="dialogDescription">
    ...
    <template #footer>
      ...
      <Button variant="outline" :disabled="isSubmitting" @click="onSubmitAndAddNew">
        <Icon
          v-if="isSubmitting"
      ...
      <Button :disabled="isSubmitting" @click="onSubmitAndClose">
        <Icon
          v-if="isSubmitting"
          name="solar:refresh-linear"
          class="mr-2 h-4 w-4 animate-spin"
